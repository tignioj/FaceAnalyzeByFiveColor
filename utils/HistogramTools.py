...
from utils.ImageUtils import getSampleDict, getImg, getcvImgFromFigure, cvshow, KEY_SAMPLE_YELLOW, KEY_SAMPLE_BLACK, \
    KEY_SAMPLE_WHITE, KEY_SAMPLE_RED, COLOR_SAMPLE_CN_NAME_BY_KEY
...
def getDistanceByDifferentColorSpace(fig, predict=None, keyName=None):
    """
    ...
    sampleDict = getSampleDict()

    black_sample_dict, yellow_sample_dict, red_sample_dict, white_sample_dict = \
        sampleDict[KEY_SAMPLE_BLACK], sampleDict[KEY_SAMPLE_YELLOW], sampleDict[KEY_SAMPLE_RED], sampleDict[
            KEY_SAMPLE_WHITE]
    # 要预测的图片
    ...
    _drawHistByDistance(labs, lab_color, order, 5, 2, 'Lab')
    _drawHistByDistance(rgbs, rgb_color, order, 5, 3, 'RGB')
    _drawHistByDistance(hsvs, hsv_color, order, 5, 4, 'HSV')
    _drawHistByDistance(ycrcbs, ycrcbs_color, order, 5, 5, 'YCrCb')

    LogUtils.log("HistogramTools", "完成'" + keyNameCN + "'的绘制")
    colorResult = getMostFrequentWord([
        lab_color, hsv_color, rgb_color, ycrcbs_color
    ])
    colorCNName = COLOR_SAMPLE_CN_NAME_BY_KEY[colorResult]
    ...
    if colorResult == KEY_SAMPLE_RED:
        sample = sampleRed
        sampleTrim = sampleRedTrim
    elif colorResult == KEY_SAMPLE_BLACK:
        sample = sampleBlack
        sampleTrim = sampleBlackTrim
    elif colorResult == KEY_SAMPLE_YELLOW:
        sample = sampleYellow
        sampleTrim = sampleYellowTrim
    elif colorResult == KEY_SAMPLE_WHITE:
        sample = sampleWhite
    ...
    return getcvImgFromFigure(fig), colorResultWithImg
...
if __name__ == '__main__':
    roi = getImg("../result/black/ting_trim.jpg")
    fig = plt.figure(figsize=(10, 10))
    img, color = getDistanceByDifferentColorSpace(fig, roi, KEY_ting)
    cvshow(img)
