...
def face_color(faceskinimage, faceblock):
    ...
    k = 0
    L_value, A_value, B_value = 0, 0, 0
    for value_r in img_lab:
        for value in value_r:
            if value[0] != 0:
                k = k + 1
                L_value = L_value + value[0]
                A_value = A_value + value[1]
                B_value = B_value + value[2]
    L0 = int(round(L_value / k))
    A0 = int(round(A_value / k))
    B0 = int(round(B_value / k))
