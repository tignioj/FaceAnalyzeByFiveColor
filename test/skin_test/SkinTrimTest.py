...
def testVideo():
    ...
    while videoCapture.isOpened():
        ...
        iteration = 0
        blur = (0, 0)
        cv2.imshow('rgb', SkinTrimUtilsTest.rgb(frame))
        cv2.imshow('hsv', SkinTrimUtilsTest.hsv(frame))
        cv2.imshow('yCrCb', SkinTrimUtilsTest.YCrCb(frame))
        cv2.imshow('lab', SkinTrimUtilsTest.Lab(frame))
        cv2.imshow('rgb_hsv_ycbcr', SkinTrimUtilsTest.rgb_hsv_ycbcr(frame))
...
def testImage():
    ...
    # cv2.imshow('rgb result', SkinTrimUtilsTest.rgb(frame.copy()))
