...
from hsv_rgb_ycrcb import Skin_Detect

videoCapture = cv2.VideoCapture(1)


def maskAndErodeAndDilateAndSmooth(colorSpaceImg, min, max):
    """
    掩码后，执行收缩膨胀，最后平滑处理
    :param colorSpaceImg:
    :param min:
...
def rgb_hsv_ycbcr(image):
    ...
    :return:
    """
    skinMask = sd.RGB_H_CbCr(image, False)
...
def rgb(image):
    ...
    skinRGB = maskAndErodeAndDilateAndSmooth(imageRGB, min_rgb, max_rgb)
...
def Lab(image):
    ...
    skinLab = maskAndErodeAndDilateAndSmooth(imageLab, min_Lab, max_Lab)
...
def hsv(image):
    ...
    skinHSV = maskAndErodeAndDilateAndSmooth(imageHSV, min_HSV, max_HSV)
...
def YCrCb(image):
    ...
    skinYCrCb = maskAndErodeAndDilateAndSmooth(imageYCrCb, min_YCrCb, max_YCrCb)
...
if __name__ == '__main__':
    while videoCapture.isOpened():
        ...
        cv2.imshow('melt', rgb_hsv_ycbcr(frame))
